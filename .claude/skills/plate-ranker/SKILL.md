...
3. **Answer verdict first:** "Best: ₹X as you asked / band top / band bottom — N stocks,
   ₹Y left over", then WHY IT WINS in plain words, then the side-by-side table and what
   each plate buys. If the ranker says NO ACTION (every variant broke a law), say "do
   not trust today's plate" and suggest the `simulate` skill.
...
5. Say the ranking order comes from policy `ranker_criteria_order` (ticket band → no
   raised budget → breadth 8-15 → least left over; ties → smaller ticket).
