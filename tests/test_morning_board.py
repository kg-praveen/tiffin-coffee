...
CI runs with NO NETWORK. Prices are mocked; the real DB (pattaz.db) is used
...
class TestMorningBoardIntegration:
    ...
    def test_board_runs_and_writes_session(self, _mock: object) -> None:
        result = run_morning_board(DB_PATH)
        assert result.run_id.startswith("UC1_")
        assert result.ran_at != ""
        assert len(result.rules_fired) > 0

    @patch("usecases.morning_board.fetch_price", side_effect=mock_fetch)
    def test_petronet_fires_at_370(self, _mock: object) -> None:
    ...
    @patch("usecases.morning_board.fetch_price", side_effect=mock_fetch)
    def test_hdfcbank_fires_crash_shelf(self, _mock: object) -> None:
    ...
    @patch("usecases.morning_board.fetch_price", side_effect=mock_fetch)
    def test_inactive_triggers_in_not_armable(self, _mock: object) -> None:
    ...
    @patch("usecases.morning_board.fetch_price", side_effect=mock_fetch)
    def test_format_output_contains_key_sections(self, _mock: object) -> None:
        result = run_morning_board(DB_PATH)
    ...
    @patch("usecases.morning_board.fetch_price", side_effect=mock_fetch)
    def test_drops_list_populated(self, _mock: object) -> None:
        result = run_morning_board(DB_PATH)
    ...
    @patch("usecases.morning_board.fetch_price", side_effect=mock_fetch)
    def test_no_fetch_mode(self, _mock: object) -> None:
        """fetch_prices=False skips price fetching; board has no entries."""
        result = run_morning_board(DB_PATH, fetch_prices=False)
...
class TestClassificationAccuracy:
    ...
    @patch("usecases.morning_board.fetch_price", side_effect=mock_fetch)
    def test_near_trigger(self, _mock: object) -> None:
    ...
    @patch("usecases.morning_board.fetch_price", side_effect=mock_fetch)
    def test_far_trigger(self, _mock: object) -> None:
